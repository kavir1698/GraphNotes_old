...
from PyQt5.QtWidgets import (QGridLayout, QLabel, QLineEdit, QMenu, QAction,
                             QDialog, QPushButton, QFileDialog, QListWidget,
                             QCompleter, QTextEdit, QComboBox, QGroupBox, QSplitter)

from bibtex import parse_bibtex_from_file
from db_managment import Concept, Subcategory, Relation, ConceptReference, Graph, RelationReference, Description
...
class EditReferenceOrDescription(QDialog):
    ...
    def _configWidgets(self):
        ...
        self.resize(600, 10)
        self.text_line.setText(self.obj.text)
    ...
    def getValue(self):
        if self.return_class == Description:
            return self.return_class(id=self.obj.id, text=self.text_line.toPlainText(), subcategory_id=self.obj.subcategory_id)
        return self.return_class(id=self.obj.id, text=self.text_line.toPlainText(), node_id=self.obj.node_id)
...
class EditRelationDialog(QDialog):
    def __init__(self, parent, relation):
        ...
        self.relation_line = QTextEdit(self)
    ...
    def _editReference(self):
        if self.references_line.currentIndex().isValid():
            ...
            if dlg.exec_():
                ...
                self.references_line.selectedItems()[0].setText(reference.text)
    ...
    def getValue(self):
        description = self.relation_line.toPlainText()
...
class EditConceptDialog(QDialog):
    ...
    def _configWidgets(self):
        ...
        self.description_field.itemClicked.connect(self.showReferences)
        ...
        self.description_field.customContextMenuRequested.connect(self.showDescriptionMenu)
    ...
    def editReference(self):
        if self.references_field.currentIndex().isValid():
            ...
            if dlg.exec_():
                ...
                self.references_field.selectedItems()[0].setText(reference.text)
                self.parent().db.update_reference(reference)
    ...
    def editDescription(self):
        if self.description_field.currentIndex().isValid():
            ...
            if dlg.exec_():
                ...
                self.description_field.selectedItems()[0].setText(description.text)
                self.parent().db.update_description(description)
    ...
    def showDescriptionMenu(self, pos):
    ...
    def commitChanges(self):
        ...
        if not relation.isEmpty():
            if node1.concept.isEmpty() or node2.concept.isEmpty():
                self.showError("Concept field is empty!")
                return
            else:
            ...
            if not node1.isEmpty() and node1.concept.isEmpty():
                self.showError("Left concept field is empty!")
                return
            if not node2.isEmpty() and node2.concept.isEmpty():
                self.showError("Right concept filed is empty!")
                return
            if node1.isEmpty() and node1.concept.isEmpty():
                self.db.delete(node1)
                self.db.delete(concept1)
            if node2.isEmpty() and node2.concept.isEmpty():
...
class ConceptBox(QGroupBox):
    ...
    def deleteReference(self):
        if self.reference_field.currentIndex().isValid():
    ...
    def getData(self):
        ...
        concept_name = self.concept_filed.text()
        subcat = self.subcategory_field.text()
        syn = self.synonyms_field.text()
        description = Description(text=self.description_field.toPlainText())
...
class RelationBox(QGroupBox):
    def __init__(self):
        ...
        # =======================
        self.relation_label = QLabel("Relation", self)
        self.relation_field = QTextEdit(self)
    ...
    def getData(self):
        relation = self.relation_field.toPlainText()
        study = self.study_field.text()
...
class NewReferenceDialog(QDialog):
    ...
    def setFromFile(self):
        ...
        if not (file_path == ""):
            # todo: check parse_BiBteX_from_file
            self.result_text += parse_bibtex_from_file(path=file_path)
    ...
    def _configWidgets(self):
        ...
        self.grid.addWidget(self.frame, 1, 0)
        self.grid.addWidget(self.apply_button, 2, 0, Qt.AlignRight)
        self.setLayout(self.grid)
...
class ReferenceGroup(QGroupBox):
    def __init__(self):
        super().__init__()
        self.last_row = 1
        self.keys = ["author", "title", "journal", "year", "pages", "publisher",
                     "address", "editor", "booktitle", "institution", "note"]
        self.fields = []
...
class SearchReference(QDialog):
    ...
    def applySelection(self):
        if self.result_list.currentIndex().isValid():
            item = self.result_list.selectedItems()[0].data(Qt.UserRole).text
...
if __name__ == '__main__':
    ...
    graph = Graph(r"C:\Users\YevheniiHyzyla\Desktop\Freelance\new.db")
